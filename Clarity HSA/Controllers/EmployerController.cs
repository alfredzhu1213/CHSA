    ...
    public class EmployerController : Controller
    {
                            ...
                            if ((string)System.Web.HttpContext.Current.Session["employee_search"] == "" || (user.first_name + " " + user.last_name).Contains((string)System.Web.HttpContext.Current.Session["employee_search"]))
                            {
                                if (!employeeIds.Contains(user.id))
                                {
                                    ...
                                    foreach (Loan loan in user.employee.loans)
                                    {
                                        ...
                                        //-----Comment By @kundan Kumar on 24 Feb 2021------------------------//
                                        //user.employee.balance += loan.balance;
                                        //user.employee.monthly_payment += loan.monthly_payment;
                                        //----Comment By @kundan Kumar on 24 Feb 2021------------------------//   

                                        #region Fetch data from Database division and Payment @kundan                                        
                                        string sql = "getEmployeeRepayment";
                                        string strcon = ConfigurationManager.ConnectionStrings["myConnection"].ConnectionString;
                                        using (SqlConnection conn = new SqlConnection(strcon))
                                        {
                                            SqlCommand cmd = new SqlCommand(sql, conn);
                                            cmd.CommandType = CommandType.StoredProcedure;
                                            cmd.Parameters.AddWithValue("@empID", user.employee.id);
                                            cmd.Parameters.AddWithValue("@organizationID", organization.id);
                                            cmd.Parameters.AddWithValue("@Debug", 1);
                                            try
                                            {
                                                conn.Open();
                                                using (DataSet ds = new DataSet())
                                                {
                                                    ...
                                                    if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                                                    {
                                                        ...
                                                        {
                                                            loan.balance = (Convert.ToDouble(ds.Tables[0].Rows[i]["Balance"]));
                                                            loan.monthly_payment = (Convert.ToDouble(ds.Tables[0].Rows[i]["RepaymentAmount"]));
                                                            user.employee.balance = loan.balance;
                                                            user.employee.monthly_payment = loan.monthly_payment;
                                                        }
                                                    }
        ...
        public ActionResult Rules()
        {
            ...
            else
            {
                ...
                var Repayment = (from o in context.RepaymentTables where o.defaultVal == "Yes" select o).ToList();
                if (organization.organization_settings == null)
                {
                    organization.organization_settings = new OrganizationSettings();
                    organization.organization_settings.alegeus_feed = "";
                    ...
                    organization.organization_settings.deposits_feed = "";
                    organization.organization_settings.withhold_entire_balance = true;
                    organization.organization_settings.repayment_table = Repayment;
                    context.SaveChanges();
                }
                ViewData["organization"] = organization;
                //Comment By Dilshad on 27-Feb 2021 ---// ViewData["organization"] = new Organization();
            }
        ...
        public ActionResult SaveRepaymentTable(string repayments_json)
        {
            ...
            if (organization_id > 0)
            {
                ...
                foreach (JObject result in resultObjects)
                {
                    ...
                    foreach (JProperty property in result.Properties())
                    {
                        ...
                        else if (property.Name == "division")
                        {
                            repayment.division = property.Value.ToString();
                        }
                        else if (property.Name == "frequency")
                        {
                            repayment.frequency = property.Value.ToString();
                        }

                    }
                    repayment.defaultVal = "No";
            ...
            if (organization_id == -1)
            {
                ...
                var Repayment = (from o in context.RepaymentTables where o.defaultVal == "Yes" select o).ToList();
                var resultObjects = AllChildren(JObject.Parse(repayments_json)).First(c => c.Type == JTokenType.Array && c.Path.Contains("repayments")).Children<JObject>();
                List<RepaymentTable> lstrepayment = new List<RepaymentTable>();
                foreach (JObject result in resultObjects)
                {
                    RepaymentTable repayment = new RepaymentTable();
                    foreach (JProperty property in result.Properties())
                    ...
                    repayment.defaultVal = "Yes";
                    lstrepayment.Add(repayment);
                }
                DataTable DatatableType = new DataTable();
                List<RepaymentTable> Lst2Repayment = new List<RepaymentTable>();
                Lst2Repayment = Repayment;
                // Adding Columns    
                DataColumn COLUMN = new DataColumn();
                COLUMN.ColumnName = "Id";
                COLUMN.DataType = typeof(int);
                DatatableType.Columns.Add(COLUMN);
                ...
                foreach (RepaymentTable Ret in Lst2Repayment)
                {
                    DataRow DR = DatatableType.NewRow();
                    DR[0] = Ret.id;
                    DR[1] = "RepaymentId";
                    DatatableType.Rows.Add(DR);
                }
                DatatableType.AcceptChanges();

                using (DataTable dt = CreateDataTable(lstrepayment))
                {

                    string sql = "defaultRepayment";
                    string strcon = ConfigurationManager.ConnectionStrings["myConnection"].ConnectionString;
                    using (SqlConnection conn = new SqlConnection(strcon))
                    {
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@list", DatatableType);
                        cmd.Parameters.AddWithValue("@listRepayment", dt);
                        cmd.Parameters.AddWithValue("@Debug", 1);
                        try
                        {
                            conn.Open();
        ...
        public ActionResult RunActualReport(Report report_to_run)
        {
            ...
            //if (Session["active_organization"] != null)
            //{
            //    organization_id = (int)Session["active_organization"];
            //}
            //else
            ...
            //}
            //---Add by Smart Data-------------//
            #region Commneted by Smart Data and Put new code with Changes
            ...
            //----This code is used for Hide the goal for other Organisation
            //if (report_to_run.output_format == "csv")
            //{
            //    var csv = new CsvWriter(tw);
            //    foreach (ReportField field in report_to_run.fields)
            ...
            //                    if (field.format == "Numeric")
            //                    {
            //                        csv.WriteField(String.Format("{0}", Convert.ToDouble(subtotals[k])));
            //                    }
            //                    else if (field.format == "Currency")
            //                    {
            //                        csv.WriteField(String.Format("{0:C}", Convert.ToDouble(subtotals[k])));
            ...
            //                if (field.format == "Numeric")
            //                {
            //                    csv.WriteField(String.Format("{0}", Convert.ToDouble(totals[k])));
            //                }
            //                else if (field.format == "Currency")
            //                {
            //                    csv.WriteField(String.Format("{0:C}", Convert.ToDouble(totals[k])));
            ...
            //                        if (field.format == "Numeric")
            //                        {
            //                            line += String.Format("{0}", Convert.ToDouble(subtotals[k]));
            //                        }
            //                        else if (field.format == "Currency")
            //                        {
            //                            line += String.Format("{0:C}", Convert.ToDouble(subtotals[k]));
            ...
            //                if (field.format == "Numeric")
            //                {
            //                    line += String.Format("{0}", Convert.ToDouble(totals[k]));
            //                }
            //                else if (field.format == "Currency")
            //                {
            //                    line += String.Format("{0:C}", Convert.ToDouble(totals[k]));
        ...
        public static DataTable CreateDataTable<T>(IEnumerable<T> list)
        {
            ...
            {
                if (info.Name == "organization_settings")
                {
                    DataColumn COLUMN = new DataColumn();
                    COLUMN.ColumnName = "organization_settings_id";
                    COLUMN.DataType = typeof(string);
                    dataTable.Columns.Add(COLUMN);
                }
                else { dataTable.Columns.Add(new DataColumn(info.Name, Nullable.GetUnderlyingType(info.PropertyType) ?? info.PropertyType)); }
            ...
            foreach (T entity in list)
            {
                ...
                {
                    if (properties[i].Name == "organization_settings")
                    { values[i] = 0; }
                    else
                    {
